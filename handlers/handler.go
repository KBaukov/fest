...
func ServeUserAuth(w http.ResponseWriter, r *http.Request) {
	err := r.ParseForm()
	if err != nil {
		http.Error(w, "Ошибка обработки запроса", http.StatusInternalServerError)
		log.Printf("Ошибка получения формы: %v", err)
	}
	regData := new(ent.UserAuthDataset) // Person being a struct type
	decoder := schema.NewDecoder()
	err = decoder.Decode(regData, r.Form)
	if err != nil {
		http.Error(w, "Ошибка обработки запроса", http.StatusOK)
		log.Printf("Ошибка получения формы: %v", err)
	}

	log.Printf("incoming Auth user request in: %v - ref: %v", r.URL.Path, regData)
...
func ServeUserCreate(w http.ResponseWriter, r *http.Request) {
	...
	regData := new(ent.UserRegDataset) // Person being a struct type
	decoder := schema.NewDecoder()
	err = decoder.Decode(regData, r.Form)
	if err != nil {
		http.Error(w, "Ошибка обработки запроса", http.StatusOK)
	...
	log.Printf("incoming create user request in: %v - ref: %v", r.URL.Path, regData)

	ok, err := db.CreateUser(regData.Name, regData.Email)
	if !ok {
		http.Error(w, "Ошибка обработки запроса", http.StatusOK)
	}
...
func ServeApi(w http.ResponseWriter, r *http.Request) {
	...
	person := new(ent.User) // Person being a struct type
	decoder := schema.NewDecoder()
	...
	log.Printf("incoming request in: %v - ref: %v", r.URL.Path, person)
	...
	//	seatMap, err := dao.GetSeats(eventId, tarif)
	//	if err != nil {
	//		http.Error(w, "Ошибка обработки запроса", http.StatusInternalServerError)
	...
	//	var leadId string
	//	orderNumber, amount, _, items, err := db.CreateOrder(name, email, phone, seatIds, eventId, utmJson, refId)
	...
	//
	//	orderNumber, amount, _, items, err := db.UpdateOrder(orderNumber, name, email, phone, seatIds, eventId, utm, amoData, refId)
	//	if err != nil {
	//		http.Error(w, "Ошибка обработки запроса", http.StatusInternalServerError)
	//		log.Printf("Ошибка создания заказа (seatIds: %v ): %v", seatIds, err)
	//	} else {
